...
class NeRFAll(nn.Module):
    def __init__(self, args, kernelsnet=None):
        super().__init__()
        self.args = args
        self.embed_fn, self.input_ch = get_embedder(args.multires, args, args.i_embed) # now support hash
        self.input_ch_views = 0
        self.kernelsnet = kernelsnet
        self.embeddirs_fn = None
        if args.use_viewdirs:
            self.embeddirs_fn, self.input_ch_views = get_embedder(args.multires_views, args, args.i_embed_views) # now support spherical

        self.output_ch = 5 if args.N_importance > 0 else 4
        skips = [4]

        # when hash, use small nerf
        if args.i_embed == 1:
            self.mlp_coarse = NeRFSmall(num_layers=2,
                        hidden_dim=64,
                        geo_feat_dim=15,
                        num_layers_color=3,
                        hidden_dim_color=64,
                        input_ch=self.input_ch, input_ch_views=self.input_ch_views)
        else:
        ...
        if args.N_importance > 0:
            if args.i_embed == 1:
                self.mlp_fine = NeRFSmall(num_layers=2,
                            hidden_dim=64,
                            geo_feat_dim=15,
                            num_layers_color=3,
                            hidden_dim_color=64,
    ...
    def raw2outputs(self, raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
        ...
        # dists = torch.cat([dists, torch.tensor([1e10]).expand(dists[..., :1].shape)], -1) # why delete this line ?

        dists = dists * torch.norm(rays_d[..., None, :], dim=-1)

        rgb = self.rgb_activate(raw[..., :3]) # [N_rays, N_samples, 3]
        noise = 0.
        if raw_noise_std > 0.:
        ...
        alpha = torch.cat([alpha, torch.ones_like(alpha[:, 0:1])], dim=-1) # [N_rays, N_samples]
    ...
    def render_rays(self,
        ...
        rgb_map, density_map, acc_map, weights, depth_map, sparsity_loss = self.raw2outputs(raw, z_vals, rays_d, raw_noise_std,
        ...
        ret = {'rgb_map': rgb_map, 'depth_map': depth_map, 'acc_map': acc_map, 'density_map': density_map, 'sparsity_loss': sparsity_loss}
        if retraw:
            ret['raw'] = raw
        if N_importance > 0:
            ret['rgb0'] = rgb_map_0
    ...
    def forward(self, H, W, K, chunk=1024 * 32, rays=None, rays_info=None, poses=None, **kwargs):
        ...
        if self.training:
            ...
            if self.kernelsnet is not None and not force_baseline:
                ...
                if kwargs['N_importance'] > 0:
                    ngp_loss['sparsity_loss0'] = extras['sparsity_loss0']
                return self.tonemapping(rgb), self.tonemapping(extras['rgb0']), {}, ngp_loss
    ...
    def render(self, H, W, K, chunk, rays=None, c2w=None, ndc=True,
        ...
        # Batchfy and Render and reshape, 将原batchify_rays直接插入到这里
        all_ret = {}
