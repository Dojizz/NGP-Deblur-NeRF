...
from ngp.radam import RAdam
from ngp.loss import sigma_sparsity_loss, total_variation_loss
...
def train():
    ...
    # Load only llff type data
    K = None
    if args.dataset_type == 'llff':
        # bds = bounds
        images, poses, bds, render_poses, i_test, bounding_box = load_llff_data(args, args.datadir, args.factor,
    ...
    # Cast intrinsics to right types, camera parameters
    H, W, focal = hwf
    H, W = int(H), int(W)
    hwf = [H, W, focal]
    ...
    expname = args.expname # experiment
    ...
    if args.kernel_type == 'deformablesparsekernel':
        kernelnet = DSKnet(args, len(images), torch.tensor(poses[:, :3, :4]),
    ...
    # For random ray batching by default
    print('get rays')
    ...
    train_datas['rays'] = rays[i_train].reshape(-1, 2, 3) # [N_train*H*W, ro+rd, 3]
    ...
    train_datas['rays_x'], train_datas['rays_y'] = xs[i_train].reshape(-1, 1), ys[i_train].reshape(-1, 1) # [N_train*H*W, 1]

    train_datas['rgbsf'] = images_train[i_train].reshape(-1, 3) # images_train [N_train, H, W, 3] -> [N_train*H*W, 3]
    ...
    for i in range(start, N_iters):
        ...
        rgb, rgb0, extra_loss, ngp_loss = nerf(H, W, K, chunk=args.chunk,
                                     rays=batch_rays, rays_info=iter_data,
                                     retraw=True, force_naive=i < args.kernel_start_iter,
                                     **render_kwargs_train) # extra_loss is align_loss
        ...
        # sparsity loss
        sparsity_loss = args.sparse_loss_weight * (ngp_loss['sparsity_loss'].sum() + ngp_loss["sparsity_loss0"].sum())
        loss = loss + sparsity_loss

        # Total Variation loss
        if args.i_embed == 1:
            n_levels = nerf.embed_fn.n_levels
            min_res = nerf.embed_fn.base_resolution
            max_res = nerf.embed_fn.finest_resolution
            log2_hashmap_size = nerf.embed_fn.log2_hashmap_size
            TV_loss = sum(total_variation_loss(nerf.embed_fn.embeddings[i], \
                                              min_res, max_res, \
                                              i, log2_hashmap_size, \
